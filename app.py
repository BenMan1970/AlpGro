...
st.set_page_config(page_title="Scanner Confluence Forex (Alpaca Data)", page_icon="⭐", layout="wide")
st.title("🔍 Scanner Confluence Forex Premium (Données Alpaca)")
st.markdown("*Utilisation de l'API Alpaca pour les données de marché H1*")

# Liste des paires adaptées pour Alpaca (cryptomonnaies disponibles)
PAIRS_ALPACA = [
    'BTCUSD', 'ETHUSD', 'XRPUSD', 'ADAUSD', 'BNBUSD'
]
...
def get_data_alpaca(symbol: str, timeframe: str = '4Hour', period_days: int = 15):
    ...
    try:
        ...
        bars = api.get_crypto_bars(
            ...
            exchanges=['CBSE']  # Coinbase, compatible avec Alpaca pour les cryptos
        ).df

        if bars.empty:
            st.warning(f"Alpaca: Données insuffisantes ou vides pour {symbol}.")
            print(f"Alpaca: Données insuffisantes ou vides pour {symbol}.")
            return None
        ...
        if len(bars) < 50:
            st.warning(f"Alpaca: Données insuffisantes pour {symbol} ({len(bars)} barres).")
            print(f"Alpaca: Données insuffisantes pour {symbol} ({len(bars)} barres).")
            return None

        print(f"Données pour {symbol} OK. Retour de {len(bars.dropna())} lignes après dropna.\n--- Fin get_data_alpaca {symbol} ---\n")
        return bars.dropna()
    except Exception as e:
        st.error(f"Erreur Alpaca pour {symbol}: {type(e).__name__} - {e}")
        print(f"ERREUR Alpaca pour {symbol}:\n{traceback.format_exc()}")
        return None
...
with col1:
    ...
    pair_to_debug = st.selectbox("🔍 Afficher OHLC pour:", ["Aucune"] + PAIRS_ALPACA, index=0)
    scan_btn = st.button("🔍 Scanner (Données Alpaca H1)", type="primary", use_container_width=True)
with col2:
    if scan_btn:
        ...
        if pair_to_debug != "Aucune":
            st.subheader(f"Données OHLC pour {pair_to_debug} (Alpaca):")
            debug_data = get_data_alpaca(pair_to_debug, timeframe="4Hour", period_days=5)
            ...
            st.divider()
        for i, symbol_scan in enumerate(PAIRS_ALPACA):
            pnd = symbol_scan.replace('USD', '')
            cp = (i + 1) / len(PAIRS_ALPACA)
            pb.progress(cp)
            stx.text(f"Analyse (Alpaca H1): {pnd} ({i+1}/{len(PAIRS_ALPACA)})")
            d_h1_alpaca = get_data_alpaca(symbol_scan, timeframe="4Hour", period_days=15)
            if d_h1_alpaca is not None:
                sigs = calculate_all_signals_pine(d_h1_alpaca)
                if sigs:
                    ...
                    pr_res.append({
                        ...
                        'details': {'Info': 'Calcul signaux (Alpaca) échoué'}
                    })
            else:
                pr_res.append({
                    'Paire': pnd, 'Direction': 'ERREUR DONNÉES ALPACA', 'Conf. (0-6)': 0, 'Étoiles': 'N/A',
                    'RSI': 'N/A', 'ADX': 'N/A', 'Bull': 0, 'Bear': 0,
                    'details': {'Info': 'Données Alpaca non dispo/symb invalide (logs serveur)'}
                })
        ...
        if pr_res:
            ...
            if not show_all:
                st.success(f"🎯 {len(dfd)} paire(s) avec {min_conf}+ confluence (Alpaca).")
            else:
                st.info(f"🔍 Affichage des {len(dfd)} paires (Alpaca).")
            if not dfd.empty:
                ...
                st.warning(f"❌ Aucune paire avec critères filtrage (Alpaca). Vérifiez erreurs données/symbole.")
        else:
            st.error("❌ Aucune paire traitée (Alpaca). Vérifiez logs serveur.")
